...
import { dataPath, jsonResponse, writeJsonFile } from "./files";
...
export async function getSearchSources(feature: string) {
  try {
...
export async function searchDatasetCatalog(request: Request) {
...
export async function putSearchSources(request: Request, feature: string) {
  const body = await request.json().catch(() => null);
