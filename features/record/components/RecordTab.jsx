...
function renderJsonNode(key, value, record, onToggleGeoJson, editorTabController, expandedJsonPaths, path) {
  if (value && typeof value === "object") {
    ...
    const summary = document.createElement("summary");
    summary.appendChild(renderJsonLabel(key, Array.isArray(value) ? "Array" : "Object", path, value));
  ...
  row.appendChild(renderJsonLabel(key, String(value), path, value));
  return row;
}
...
function renderHtmlElementLabel(key, element, path) {
  ...
  keyElement.className = `json-key${responsePath ? " json-copy-target" : ""}`;
  keyElement.title = responsePath ? "Right-click to copy response path" : "";
  keyElement.textContent = key;
  if (responsePath) {
    keyElement.addEventListener("contextmenu", (event) => {
      event.preventDefault();
      event.stopPropagation();
      copyJsonNodeText(keyElement, responsePath, path, element, "path");
    });
  }
...
function renderJsonLabel(key, value, path, rawValue) {
  ...
  if (responsePath) {
    keyElement.addEventListener("contextmenu", (event) => {
      event.preventDefault();
      event.stopPropagation();
      copyJsonNodeText(keyElement, responsePath, path, rawValue, "path");
    });
  }
...
function toResponsePath(path) {
  const parts = path.split(".").slice(2);
  if (parts[0] !== "response" || parts.length < 2) return "";
