...
export function createSearchSourceEditorPanel(onSaved?: () => void) {
  ...
  function createCard(source: EditableAddressSearchSource, i: number) {
    ...
    title.textContent = source.name || "New source";
    ...
    [titleInput, descInput].forEach((input) => {
      ...
      input.addEventListener("input", () => {
        ...
        title.textContent = source.name.trim() || "New source";
  ...
  function createDeletedSourceRow(source: EditableAddressSearchSource) {
    ...
    label.textContent = `Deleted: ${source.name || "Source"}`;
    ...
    revertButton.setAttribute("aria-label", `Restore ${source.name || "source"}`);
    revertButton.title = `Restore ${source.name || "source"}`;
  ...
  addBtn.addEventListener("click", () => {
    const source: EditableAddressSearchSource = { id: `src-${Date.now()}`, name: "New source", type: "geosearch", costly: false, apiKey: "", description: "", outputs: [] };
    sources.push(source);
