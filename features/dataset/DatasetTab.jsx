...
import { searchCatalog } from "../../components/search/catalogSearch";
import { buildSearchCatalogDatasetDetailPanel } from "../../components/search/CatalogPanel.jsx";
...
export function createDatasetController(recordController, builtinController, editorTabController, agentController) {
  ...
  async function loadCatalogSearchSources(widget) {
    try {
      const response = await fetch(withBasePath("/api/search?feature=dataset"));
  ...
  async function loadStaticDatasetSources() {
    try {
      const response = await fetch(withBasePath("/api/data/features/dataset.json"));
  ...
  async function loadSupportedInputParams() {
    try {
      let res = await fetch(withBasePath("/api/search?feature=dataset"));
      if (!res.ok) {
        res = await fetch(withBasePath("/api/data/search.json"));
      }
      if (!res.ok) return;
      ...
      supportedInputParamsByType = Object.fromEntries(
        (Array.isArray(catalogs) ? catalogs : []).map((catalog) => [normalizeSourceType(catalog.type), catalog.supportedInputParams || []])
      );
      redrawDatasetSources();
    } catch (error) {
  ...
  async function getSearchSourceConfig(sourceId) {
    ...
    try {
      const response = await fetch(withBasePath("/api/search?feature=address"));
