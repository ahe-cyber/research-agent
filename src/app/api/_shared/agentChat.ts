...
import { dataPath, jsonResponse } from "./files";
import { errorMessage, isHttpUrl, isJsonContentType } from "./http";
import { TOOL_DECLARATIONS } from "../tools/route";
...
async function runAgentChat({ apiKey, appMessages, legacyContents, systemInstruction, reportContent, send }: any) {
  ...
  try {
    datasetSources = JSON.parse(await readFile(datasetsPath, "utf8"));
  } catch {}
  ...
  async function executeAgentTool(name: string, args: any = {}) {
    ...
    try {
      ...
      send({ type: "search_error", query: args.query, hubName, message: errorMessage(error) });
...
async function searchArcGIS(hubUrl: string, query: string, bbox: string) {
...
async function searchArcGISPortal(base: string, query: string, bbox: string) {
...
async function searchArcGISHub(base: string, query: string) {
  ...
  return (data.data || data.results || []).map((item: any) => {
...
async function searchSocrata(hubUrl: string, query: string) {
