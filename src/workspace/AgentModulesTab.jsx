...
export function AgentModulesTab({ active }) {
  return (
    <section
...
export function createAgentModulesController(editorTabController, agentController = null) {
  ...
  canvas.className = "agents-canvas";
  canvas.style.transformOrigin = "0 0";

  // ── SVG connections layer ─────────────────────────────────────────────────

  const svgEl = document.createElementNS(SVG_NS, "svg");
  svgEl.classList.add("agents-connections-svg");
  ...
  selectionBox.className = "agents-selection-box";

  // ── Fit-to-viewport button ────────────────────────────────────────────────
  const fitBtn = document.createElement("button");
  fitBtn.className = "agents-fit-btn";
  fitBtn.type = "button";
  ...
  editButton.addEventListener("click", () => {
    editorTabController.openAgentsTab(editorPanel);
  ...
  function renderCompact() {
    ...
    if (data.agents.length === 0) {
      ...
      empty.className = "agents-compact-empty";
